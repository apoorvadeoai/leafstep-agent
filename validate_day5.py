...
docker_text = (
    (ROOT / "Dockerfile").read_text() if (ROOT / "Dockerfile").exists() else ""
)
req_text = (
...
agent_text = (
    (ROOT / "app" / "agent.py").read_text()
    if (ROOT / "app" / "agent.py").exists()
    else ""
)
tools_text = (
    (ROOT / "app" / "tools.py").read_text()
    if (ROOT / "app" / "tools.py").exists()
    else ""
)
...
checks.update(
    {
        ...
        "sustainability guardrail tool present": "sustainability_guardrail_tool"
        in tools_text,
    }
)
